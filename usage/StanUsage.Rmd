...
# pkg_query <- c('stan',        'rstan', 'rstanarm', 'shinystan', 'pystan',
#               'mc-stan.org', 'rstan', 'rstanarm', 'shinystan', 'pystan')

pkg_query <- c('pymc3', 'pymc', 'rstan', 'pystan', 'pymc2', 'pymc4',
...
for (i in 1:nrow(pkg_query_m) ) {
  ...
  for (year in year_start:year_end) {
    url <- paste("https://scholar.google.com/scholar?q=",query,
                 "&hl=en&as_sdt=0%2C33&as_ylo=",year,
                 "&as_yhi=",year,sep="")
...
day_count_current_year <- as.numeric(as.Date("2020-08-15") - as.Date('2020-01-01'))
...
df_long_lable <- df_long %>% 
                 mutate(label=if_else(years == max(years), 
                                      as.character(package),NA_character_))

plot2n <- ggplot(data=df_long_lable,aes(x=years,y=yr_count,group=package, color=package)) + 
  geom_line() +
...
log_plot <- plot2 + scale_y_continuous(breaks=c(0,2,20,150,1000),
                                        trans = scales::log_trans())
