...
cz_funded_q <- '(LIMIT-TO(FUND-SPONSOR ,  "Chan Zuckerberg Initiative" ) )'



cz_funded_and_q <- '+AND+FUND-ALL+(Chan+AND+Zuckerberg+AND+Initiative)'

cz_eco_q <- paste(pymc_arviz_stan_eco_q, cz_funded_and_q, sep='')
...
sum(df_long_label2$yr_count[df_long_label2$years==2016])
sum(df_long_label2$yr_count[df_long_label2$years==2017])
sum(df_long_label2$yr_count[df_long_label2$years==2018])
sum(df_long_label2$yr_count[df_long_label2$years==2019])
sum(df_long_label2$yr_count[df_long_label2$years==2020])
sum(df_long_label2$yr_count)



Some CRAN stuff that belongs elsewhere

library('packageRank')
rstan_ <- cranDownloads(packages = "rstan", from = "2020")
